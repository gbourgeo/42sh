...
static char * const *   ft_env_remove_from_env(const char *name, char **env)
{
    int     i;
...
static char * const *   ft_env_add_to_env(const char *name, char **env)
{
    int     i;
    ...
    while (args[i] && args[i][0] == '-')
    {
        ...
        while (args[i][j])
        {
            ...
            else if (args[i][j] == 'u')
            {
                ...
                {
                    ft_error(e, "%s: option needs argument -- %c", args[0], args[i][j]);
            ...
            else if (args[i][j] == '-')
            {
                if (j != 1)
                {
                    ft_error(e, "%s: illegal option -- %c", args[0], args[i][j]);
                    ft_dprintf(STDERR_FILENO, "use \" %s --help \" for more informations\n",
                        args[0]);
                    return (1);
                }
                ...
                {
                    ft_error(e, "%s: unrecognised option -- %s", args[0], args[i]);
                    ft_dprintf(STDERR_FILENO, "use \" %s --help \" for more informations\n",
                        args[0]);
                    return (1);
                }
...
int                 ft_env(const char **args, void *e)
{
    ...
    ft_env_init(&env);
    if (ft_env_parse_option(e, &env, args, &pos) != 0)
    {
        ft_env_free(&env);
        return (1);
    }
    ...
    if (env.change_dir != NULL)
    {
        ...
        {
            ft_error(e, "%s: cannot change directory to « %s »", args[0], env.change_dir);
            ft_env_free(&env);
            return (1);
        }
    }
    ...
    else
    {
        ...
        env.save = ((t_shell *)e)->env;
        ((t_shell *)e)->env = env.cpy;
        i = check_and_exec(&args[pos], e);
        ((t_shell *)e)->env = env.save;
    }
    ft_env_free(&env);
    return (i);
}
